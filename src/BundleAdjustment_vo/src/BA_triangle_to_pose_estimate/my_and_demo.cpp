...
int COL,ROW;
Mat K = (Mat_<double>(3, 3) << 718.8, 0, 607.1, 0, 718.8, 185.2, 0, 0, 1);

Mat all_R=Mat::eye(3,3,CV_64F);
Mat all_t=Mat::zeros(3,1,CV_64F);
...
Point2d visual_odometry::pixel2cam(const Point2d &p,const Mat &K)
{
    return Point2d(
...
void visual_odometry::image_init(const Mat &_img1,const Mat &_img2)
{
    ...
    vector<Point3f> points,points_2;
    Mat mid_points;
    ...
    for(int i=0;i<pts_4d.cols;i++)
    {
        ...
        points.push_back(p);
        mid_points=R * (Mat_<double>(3,1)<<p.x,p.y,p.z)+t;//3d points in the second img 
        points_2.push_back(Point3d(mid_points.at<double>(0),mid_points.at<double>(1),mid_points.at<double>(2))) ;
    
    }
    
    cout<<"points_2"<<points_2;
    //     // //初始化，存储最开始的3d点坐标
    ...
    // use pnp to get 3d points
    // cv::Mat pts_3d_mat(points);
    // cv::Mat pts_2d_mat(pts2);
    // Mat r,t_;
    // Mat R_;

    // solvePnP(pts_3d_mat,pts_2d_mat,K,Mat(),r,t_,false);
    // cv::Rodrigues(r,R_);
    // cout<<"R="<<R_<<endl;
    // cout<<"t="<<t_<<endl;
    ...
    Mat BA_r=(Mat_<double>(3,3)<<
        rotation_matrix(0,0),rotation_matrix(0,1),rotation_matrix(0,2),
        rotation_matrix(1,0),rotation_matrix(1,1),rotation_matrix(1,2),
        rotation_matrix(2,0),rotation_matrix(2,1),rotation_matrix(2,2)
    );
    Mat BA_t=(Mat_<double>(3,1)<<translation_vector(0),translation_vector(1),translation_vector(2));
    ...
    all_R=BA_r*all_R;
    // all_R=all_R*BA_r;
    // all_t=all_t+all_R*BA_t;
    cout<<"===all_R======"<<all_R<<endl;
    cout<<"$$$$all_t$$$$$$$"<<all_t<<endl;
    cout<<"@@@@@@@@@@@@@@@@@@@@@@@@@@@@@2"<<BA_r<<endl;
...
void visual_odometry::image_test(const Mat &_img1,const Mat &_img2)
{
    vector<uchar> status;
    vector<float> err;
    vector<Point2f> pts1,pts2;
...
void visual_odometry::newImage_process(const Mat &cur_img,int nframe)
{
    ...
    cout<<"###BA之后的pts2-2d点size####:"<<cur_pts.size()<<endl;
