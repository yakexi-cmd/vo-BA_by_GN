...
void pose_estimation_2d2d(
    ...
    Mat &R,Mat &t)
{
    Point2f principal_point(363.0,248.1);//325.1,249.7);//对应像素平面（单位为pixel）和成像平面[归一化平面，单位为m]的关系，二者之间是单位的区别
    int focal_length=461.6;//521;//对应焦距
...
void readImage(const Mat &_img)
{
    ...
    if(init_img == true)
    {
        ...
        clahe->apply(forw_img,forw_img);
        // feature_detect_lk(img1,img2);
        
        if(init_structure )
        {
            image_init(cur_img,forw_img);
        ...
        else
        {
            if(i_%10==0)
            {
                image_process(all_img_buf,forw_img);
            }
            ...
            // else
                newImage_process(all_img_buf,forw_img);
        }
    }
    cur_img=forw_img;
}
