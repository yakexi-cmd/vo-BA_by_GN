...
// // vector<Point3f> points_buf;//存储前一张图对应的空间点坐标
// VecVector3d points_buf_eigen;
// Mat img_buf;
...
//     {
//         points_buf_eigen.push_back(T*points_3d[i]);
//     }
//     cout<<T.matrix()<<endl;
...
//     cv::calcOpticalFlowPyrLK(_img1,_img2,pts_buf,pts2,status,err,cv::Size(21,21),3);
//     Mat K = (Mat_<float>(3, 3) << 520.9, 0, 325.1, 0, 521.0, 249.7, 0, 0, 1);
//     // vector<Point2f> pts2_cam;
//     // for(int i=0;i<pts2.size();i++)
//     // {
//     //     pts2_cam.push_back(pixel2cam(pts2[i],K));
//     // }
...
//     {
//         // pts_3d_eigen.push_back(Eigen::Vector3d(points_buf[i].x,points_buf[i].y,points_buf[i].z));
//         pts_2d_eigen.push_back(Eigen::Vector2d(pts2[i].x,pts2[i].y));
//     }
...
//     // //保存前一张图中的信息，用于和后一张图进行BA计算
//     bundleAdjustment_gaussNewton(points_buf_eigen,pts_2d_eigen,K,pose_gn);
...
//         if (status[i]) {
//             cv::circle(img2_CV, pts2[i], 2, cv::Scalar(0, 250, 0), 2);
//             cv::line(img2_CV, pts_buf[i], pts2[i], cv::Scalar(0, 250, 0));
//         }
//     }
//     imshow("img2_CV",img2_CV);
//     _img2.copyTo(img_buf);
...
//             {
//                 // image_process(img1,img2);
//                 imshow("img_buf",img_buf);
//                 newImage_process(img_buf,img2);
//             }
//             waitKey(0);
